...
msg-profile =
    <b>👤 Профиль:</b>
    <blockquote>
    • ID: <code>{ NUMBER($telegram_id, useGrouping: 0) }</code>
    • Имя: { $name }
    </blockquote>
    ...
    *[0]
    ...
    Купите подписку или пригласите друзей и получите 7 дней VPN за каждого приглашённого друга.
    }
...
msg-menu-invite-about =
    ...
    [ON_TRIAL_ACTIVATION] Награда начисляется, когда приглашенный пользователь активирует пробный период.
    *[OTHER] { $accrual_strategy }
    }
    </blockquote>

msg-menu-about-project =
    <b>🚀 О проекте q1 vpn</b>

    q1 vpn - это сервис для стабильного, быстрого и удобного подключения к интернету без лишних ограничений.
    Мы сделали всё просто: одна подписка, быстрый запуск и понятное управление через Telegram-бот.

    <b>🔒 Конфиденциальность</b>
    Мы не храним историю посещений и не собираем данные о том, какие сайты вы открываете.
    Мы также не продаём пользовательские данные третьим лицам.

    <b>📲 Надёжный доступ</b>
    Управление подпиской и подключением работает через Telegram, поэтому доступ к сервису не завязан только на App Store или другие площадки распространения приложений.
